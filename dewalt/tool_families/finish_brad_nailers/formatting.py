...
def build_display_rows(rows: list[RowData]) -> list[RowData]:
    """Prepare finish/brad-nailer rows with UI-friendly display fields.

    Args:
        rows: Raw finish/brad-nailer row dictionaries loaded from the snapshot.
    ...
    for row in rows:
        ...
        prepared_row["gauge_display"] = format_integer_display(row.get("gauge"), " ga")
        prepared_row["magazine_angle_display"] = format_numeric(
            row.get("magazine_angle_deg"),
            " degrees",
        )
        prepared_row["magazine_capacity_display"] = format_integer_display(
            row.get("magazine_capacity")
        )
        prepared_row["fastener_max_length_display"] = format_numeric(
            row.get("fastener_max_length_in"),
            " in.",
        )
        ...
        prepared_row["jam_clearing_display"] = format_bool(row.get("jam_clearing"))
        prepared_row["tool_free_depth_adjust_display"] = format_bool(
            row.get("tool_free_depth_adjust")
        )
        prepared_row["low_nail_lockout_display"] = format_bool(
            row.get("low_nail_lockout")
        )
        prepared_row["selectable_trigger_display"] = format_bool(
            row.get("selectable_trigger")
        )
...
def compare_display_value(row: RowData, field_name: str) -> str:
    """Format a finish/brad-nailer row value for comparison and detail display.

    Args:
        row: Finish/brad-nailer row dictionary.
    ...
    if field_name == "gauge":
        return format_integer_display(value, " ga")
    if field_name == "magazine_angle_deg":
        return format_numeric(value, " degrees")
    if field_name == "magazine_capacity":
        return format_integer_display(value)
    if field_name == "fastener_max_length_in":
...
def build_stat_cards(rows: list[RowData]) -> tuple[StatCard, ...]:
    """Build the finish/brad-nailer summary cards shown in the family overview.

    Args:
        rows: Prepared finish/brad-nailer rows currently available to the dashboard.

    Returns:
        A tuple of hero statistic cards for the family.
    """
    return (
        StatCard("Finish/Brad Nailers", str(len(rows))),
        StatCard("Cordless", str(sum(1 for row in rows if row["power_source"] == "Cordless"))),
        StatCard("Pneumatic", str(sum(1 for row in rows if row["power_source"] == "Pneumatic"))),
        StatCard("Pin Nailers", str(sum(1 for row in rows if row.get("nailer_type") == "Pin Nailer"))),
    )

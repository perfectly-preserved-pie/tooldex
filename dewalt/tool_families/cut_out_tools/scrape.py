...
CATALOG_URL = (
    "https://www.dewalt.com/products/power-tools/multi-function-tools/cut-out-tools"
)
DATA_PATH = Path(__file__).resolve().parents[3] / "data" / "dewalt_cut_out_tools.json"
...
CUT_DEPTH_PATTERN = re.compile(
    r"(\d+(?:-\d+/\d+|\.\d+)?|\d+/\d+)\s*in\.\s*max depth of cut",
    re.I,
)
...
def format_fraction_label(value: float | None) -> str | None:
    ...
    Returns:
        A normalized string such as ``"3"`` or ``"1/4"``, or ``None``.
    """
    if value is None:
        return None
...
def normalize_size_label(raw_value: str | None) -> str | None:
    """Normalize an inch-based size label for cut-out-tool measurements.
...
def parse_battery_type(specs: dict[str, str]) -> str | None:
    """Parse the cut-out-tool battery chemistry label when present.
...
def parse_tool_type(title: str) -> str:
    """Parse the tool subtype from the product title.

    Args:
        title: Product title string.

    Returns:
        ``"Cut-Off Tool"`` or ``"Cut-Out Tool"``.
    """
    lowered = title.lower()
    if "cut-off tool" in lowered or "cut-off" in lowered:
        return "Cut-Off Tool"
    return "Cut-Out Tool"


def parse_collet_size_display(specs: dict[str, str]) -> str | None:
    """Parse the displayed collet-size list for cut-out tools.

    Args:
        specs: Parsed specification label-value map.

    Returns:
        A normalized collet-size display string, or ``None`` when unavailable.
    """
    raw_value = specs.get("Collet Size [in]")
    if not raw_value:
        return None

    labels = []
    for token in re.split(r"\s*,\s*", raw_value):
        label = normalize_size_label(token)
        if label:
            labels.append(f"{label} in.")
    return ", ".join(labels) if labels else None


def parse_wheel_diameter_label(specs: dict[str, str], title: str) -> str | None:
    """Parse and normalize the cut-off wheel diameter.
    ...
    Returns:
        A normalized wheel-diameter label, or ``None`` when unavailable.
    """
    raw_value = specs.get("Wheel Diameter [in]") or specs.get("Disc Diameter [in]")
    if raw_value:
        return normalize_size_label(raw_value)

    if "cut-off tool" not in title.lower():
        return None
...
def parse_rpm_max(specs: dict[str, str], all_text: str) -> int | None:
    """Parse the highest published speed value.
    ...
    """
    value = parse_int_value(specs.get("No Load Speed [RPM]"))
...
def parse_max_watts_out(all_text: str) -> int | None:
    """Parse the maximum published watts-out value from feature text.
...
def parse_max_cut_depth(all_text: str) -> float | None:
    """Parse the published maximum cut depth in inches.

    Args:
        all_text: Combined product text used for matching.

    Returns:
        The maximum cut depth in inches, or ``None`` when unavailable.
    """
    match = CUT_DEPTH_PATTERN.search(all_text)
    if not match:
        return None
...
def should_exclude_product(title: str, description: str) -> bool:
    ...
    Returns:
        ``True`` when the product is outside the intended cut-out-tool scope.
    """
    lowered = f"{title} {description}".lower()
    if not title or title == "404 Page | DEWALT":
        return True
    if "grinder" in lowered or "cut saw" in lowered:
        return True
    return not any(needle in lowered for needle in ("cut out tool", "cut-off tool", "cut-out tool"))
...
def is_tool_only_cordless(
    ...
    """Infer whether a cordless cut-out-tool listing is the bare-tool SKU.
...
def is_supported_cut_out_tool(row: dict[str, Any]) -> bool:
    """Apply the cut-out-tool product-scope rule for the dashboard.

    Args:
        row: Parsed cut-out-tool row.

    Returns:
        ``True`` for all corded cut-out tools and bare-tool cordless models.
    """
...
def parse_product_page(url: str, html_text: str) -> dict[str, Any] | None:
    """Parse a cut-out-tool product page into a structured row.
    ...
    wheel_diameter_label = parse_wheel_diameter_label(specs, title)
    ...
    return {
        ...
        "tool_type": parse_tool_type(title),
        "collet_size_display": parse_collet_size_display(specs),
        "wheel_diameter_label": wheel_diameter_label,
        "wheel_diameter_in": parse_measurement_value(wheel_diameter_label),
        "max_cut_depth_in": parse_max_cut_depth(all_text),
        "rpm_max": parse_rpm_max(specs, all_text),
        "max_watts_out": parse_max_watts_out(all_text),
        "tool_length_in": parse_float_value(specs.get("Product Length [in]")),
        "weight_lbs": parse_weight_lbs(specs),
        "brushless": parse_bool_feature(specs, "Is Brushless?", lowered, ("brushless",)),
        "led_light": parse_bool_feature(specs, "Has LED Light?", lowered, ("led",)),
        "tool_free_bit_change": parse_bool_feature(
            specs,
            "Has Tool-Free Bit Change?",
            lowered,
            ("tool-free bit change", "tool-free for fast and easy bit changing"),
        ),
        "axis_lock": parse_bool_feature(
            specs,
            "Has Axis Lock Indicator?",
            lowered,
            ("axis lock",),
        ),
        "dust_extraction": "airlock" in lowered or "dust shroud" in lowered,
...
def fetch_listing_pages() -> dict[str, str]:
    """Fetch all paginated cut-out-tool catalog listing pages.
...
def collect_product_cards(listing_pages: dict[str, str]) -> list[dict[str, str]]:
    """Collect unique product cards from the cut-out-tool listing pages.
...
def build_snapshot_from_live_catalog() -> dict[str, Any]:
    """Build a cut-out-tool snapshot by scraping the live DEWALT catalog.

    Args:
        None.

    Returns:
        A JSON-serializable snapshot payload for cut-out tools.
    """
    ...
    for card in product_cards:
        ...
        if parsed_row and is_supported_cut_out_tool(parsed_row):
...
def build_snapshot_from_directory(source_dir: Path) -> dict[str, Any]:
    """Build a cut-out-tool snapshot from cached product-page HTML files.
...
def save_snapshot(snapshot: dict[str, Any], output_path: Path) -> None:
    """Persist a cut-out-tool snapshot to disk as formatted JSON.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the cut-out-tool scraper.
    ...
    """
    parser = argparse.ArgumentParser(description="Scrape DEWALT cut-out-tool product data.")
    parser.add_argument(
        "--output",
        type=Path,
        default=DATA_PATH,
...
def main() -> None:
    """Run the cut-out-tool scraper CLI.
    ...
    print(f"Wrote {snapshot['product_count']} cut-out tools to {args.output}")

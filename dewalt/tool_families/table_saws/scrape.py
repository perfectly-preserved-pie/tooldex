...
CATALOG_URL = "https://www.dewalt.com/products/power-tools/saws/table-saws"
DATA_PATH = Path(__file__).resolve().parents[3] / "data" / "dewalt_table_saws.json"
...
RIP_CAPACITY_PATTERN = re.compile(
    r"(\d+(?:-\d+/\d+|\.\d+)?|\d+/\d+)\s*in\.\s*(?:of\s*)?rip capacity",
    re.I,
)
...
def format_fraction_label(value: float | None) -> str | None:
    ...
    Returns:
        A normalized string such as ``"10"`` or ``"8-1/4"``, or ``None``.
    """
    if value is None:
        return None
...
def normalize_size_label(raw_value: str | None) -> str | None:
    """Normalize an inch-based size label for table-saw measurements.
...
def parse_battery_type(specs: dict[str, str]) -> str | None:
    """Parse the table-saw battery chemistry label when present.
...
def parse_stand_type(title: str) -> str:
    """Parse the stand type from the product title.

    Args:
        title: Product title string.

    Returns:
        ``"Rolling"``, ``"Scissor"``, or ``"None"``.
    """
    lowered = title.lower()
    if "rolling stand" in lowered:
        return "Rolling"
    if "scissor stand" in lowered:
        return "Scissor"
    return "None"
...
def parse_blade_diameter_label(specs: dict[str, str], title: str) -> str | None:
    ...
    """
    raw_value = specs.get("Blade Diameter [in]") or specs.get("Disc Diameter [in]")
    if raw_value:
...
def parse_bevel_capacity(specs: dict[str, str], all_text: str) -> float | None:
    ...
    Returns:
        The bevel-capacity value, or ``None`` when unavailable.
    """
    value = parse_measurement_value(specs.get("Bevel Capacity [deg]"))
    if value is not None:
        return value

    match = re.search(r"0\s*-\s*(\d+(?:\.\d+)?)\s*°?\s*bevel capacity", all_text, re.I)
    if match:
        return float(match.group(1))
    return None


def parse_rip_capacity_right(specs: dict[str, str], all_text: str) -> float | None:
    """Parse the primary right-side rip capacity in inches.
    ...
    Returns:
        The right-side rip capacity in inches, or ``None`` when unavailable.
    """
    value = parse_measurement_value(specs.get("Max. Ripping Capacity (Right) [in]"))
    if value is not None:
        return value
    ...
    metric_value = parse_measurement_value(specs.get("Cutting Capacity [mm]"))
    if metric_value is not None:
        return round(metric_value / 25.4, 2)

    match = RIP_CAPACITY_PATTERN.search(all_text)
    if match:
        return parse_measurement_value(match.group(1))
    return None


def parse_depth_cut(specs: dict[str, str], field_names: tuple[str, ...]) -> float | None:
    """Parse a published depth-of-cut measurement in inches.

    Args:
        specs: Parsed specification label-value map.
        field_names: Candidate spec labels to check in order.

    Returns:
        The parsed depth-of-cut value, or ``None`` when unavailable.
    """
...
def parse_rpm_max(specs: dict[str, str], all_text: str) -> int | None:
    ...
    value = parse_int_value(
        specs.get("No Load Speed [RPM]") or specs.get("Speed [rpm]") or specs.get("RPM")
    )
    if value is not None:
        return value
...
def should_exclude_product(title: str, description: str) -> bool:
    ...
    Returns:
        ``True`` when the product is outside the intended table-saw scope.
    """
    lowered = f"{title} {description}".lower()
    if not title or title == "404 Page | DEWALT":
        return True
    if "blades" in lowered or "blade" in lowered and "table saw" not in lowered:
        return True
    return "table saw" not in lowered
...
def is_tool_only_cordless(
    ...
    """Infer whether a cordless table-saw listing is the bare-tool SKU.
...
def is_supported_table_saw(row: dict[str, Any]) -> bool:
    """Apply the table-saw product-scope rule for the dashboard.

    Args:
        row: Parsed table-saw row.
...
def parse_product_page(url: str, html_text: str) -> dict[str, Any] | None:
    """Parse a table-saw product page into a structured row.
    ...
    return {
        ...
        "stand_type": parse_stand_type(title),
        ...
        "rip_capacity_right_in": parse_rip_capacity_right(specs, all_text),
        "depth_cut_90_in": parse_depth_cut(
            specs,
            ("Max. Depth of Cut at 90° [in]", "Max. Depth of Cut at 90 degrees [in]"),
        ),
        "depth_cut_45_in": parse_depth_cut(
            specs,
            ("Max. Depth of Cut at 45° [in]", "Max. Depth of Cut at 45 degrees [in]"),
        ),
        ...
        "dust_extraction": parse_bool_feature(
            ...
            "Has Dust Extraction?",
            lowered,
            ("dust collection", "dust extraction", "dust port"),
        ),
        "rack_and_pinion_fence": "rack & pinion" in lowered or "rack and pinion" in lowered,
        "blade_brake": "blade brake" in lowered,
        "onboard_storage": "on-board storage" in lowered or "onboard storage" in lowered,
        "power_loss_reset": "power-loss reset" in lowered or "power loss reset" in lowered,
...
def fetch_listing_pages() -> dict[str, str]:
    """Fetch all paginated table-saw catalog listing pages.
...
def collect_product_cards(listing_pages: dict[str, str]) -> list[dict[str, str]]:
    """Collect unique product cards from the table-saw listing pages.
...
def build_snapshot_from_live_catalog() -> dict[str, Any]:
    """Build a table-saw snapshot by scraping the live DEWALT catalog.

    Args:
        None.

    Returns:
        A JSON-serializable snapshot payload for table saws.
    """
    ...
    for card in product_cards:
        ...
        if parsed_row and is_supported_table_saw(parsed_row):
...
def build_snapshot_from_directory(source_dir: Path) -> dict[str, Any]:
    """Build a table-saw snapshot from cached product-page HTML files.
...
def save_snapshot(snapshot: dict[str, Any], output_path: Path) -> None:
    """Persist a table-saw snapshot to disk as formatted JSON.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the table-saw scraper.
    ...
    """
    parser = argparse.ArgumentParser(description="Scrape DEWALT table-saw product data.")
    parser.add_argument(
        "--output",
        type=Path,
        default=DATA_PATH,
...
def main() -> None:
    ...
    print(f"Wrote {snapshot['product_count']} table saws to {args.output}")
